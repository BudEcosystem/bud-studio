...
import { createNewFile, createWorkspace } from "actions/pages";
...
export default function SideMenuLayout({ children, showTopBar }) {
  ...
  const newPage = async () => {
    ...
    push(
      `/projects/view?page=${file.fileName.split(".")[0]}&filePath=${
        file.filePath
      }`
    );
  };
  ...
  useEffect(() => {
    if (fileSystem) {
      setWorkspaces(fileSystem);
    }
  }, []);

  useEffect(() => {
    console.log("On Frontend ", fileSystem);
    if (fileSystem) {
      setWorkspaces(fileSystem);

      if (selectedWokspace) {
        const resultObj = fileSystem.find(
          (obj) => obj.name === selectedWokspace.name
        );
        setSelectedWorkspace(resultObj);
      }
    }
  }, [fileSystem]);
  ...
  return (
    <div
      className={
        collapsed
          ? `${styles.layoutWrapper} ${styles.collapsed}`
    ...
    >
      <div className={styles.sidePanelWrapper}>
        ...
        <div className={styles.workspacesWrap}>
          ...
          <div className={styles.workspaceListWrap}>
            ...
            {showAddWorkspace && (
              <div className={styles.workspaceItem}>
                <div
                  className={styles.colorTag}
                  style={{ backgroundColor: `${generateColor("random")}` }}
                ></div>
                <input
                  type="text"
          ...
          {/* TODO : change the base path to state */}
          {showWorkspaceViewer && (
            <WorkspaceViewer
              refreshStorage={refreshStorage}
              getBasePath={baseSavePath}
              selectedWokspace={selectedWokspace}
            ></WorkspaceViewer>
          )}
        </div>
      </div>
